...
*
* Descrição: Informações Interfaces BOPF
*
...
*      gt_int_mess   TYPE zcac0004_k,
      ...
      gt_root       TYPE TABLE OF ty_root,
*      gt_mess       TYPE TABLE OF ty_mess,
...
START-OF-SELECTION.
  ...
  PERFORM f_get_data.
  PERFORM f_display_alv USING abap_true CHANGING gt_root.
...
FORM f_get_data.
  ...
  PERFORM f_build_sel_para CHANGING gt_parameters.
...
*       et_messages   = gt_int_mess
  ...
  DATA: ls_root       LIKE LINE OF gt_root.

  DELETE gt_int_root  WHERE erdat NOT IN s_dtcri.
  DELETE gt_int_root  WHERE erzet NOT IN s_hrcri.

  LOOP AT gt_int_root ASSIGNING FIELD-SYMBOL(<root>).

    MOVE-CORRESPONDING <root> TO ls_root.

*    READ TABLE gt_int_root INTO DATA(int_root) WITH KEY key = <root>-root_key.
*    CHECK sy-subrc = 0.

*    MOVE-CORRESPONDING int_root      TO ls_data.

    DATA(stainfo) = zcacl_process_interface=>get_status_info( ls_root-status ).

    MOVE-CORRESPONDING stainfo TO ls_root.

    ls_root-key     = <root>-key.

    APPEND ls_root TO gt_root.
    CLEAR ls_root.
  ENDLOOP.

  SORT gt_root BY id_number.

ENDFORM.

FORM f_build_sel_para   CHANGING ct_parameters TYPE /bobf/t_frw_query_selparam.
...
FORM user_command USING ucomm    TYPE sy-ucomm
  ...
  CASE ucomm.
    ...
    WHEN 'MESS'.
      READ TABLE gt_root INTO DATA(root) INDEX selfield-tabindex.

      DATA(lt_messages) = zcacl_process_interface=>get_messages( root-key ).
...
FORM f_display_data USING p_index.
  ...
  READ TABLE gt_root INTO DATA(root) INDEX p_index.

  CHECK sy-subrc = 0.

  READ TABLE gt_int_data INTO DATA(data) WITH KEY root_key = root-key.
